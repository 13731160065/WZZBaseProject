...
- (void)createUI {
    ...
    if (self.text && !self.image) {
        ...
        if (self.labelMaxWidth) {
            [NSLayoutConstraint constraintWithItem:self.label attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1 constant:self.labelMaxWidth.doubleValue].active = YES;
        }
    } else if (!self.text && self.image) {
        //仅图片
        [self addSubview:self.imageView];
        [NSLayoutConstraint constraintWithItem:self.image attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeCenterY multiplier:1 constant:0].active = YES;
        [NSLayoutConstraint constraintWithItem:self.image attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1 constant:self.imageWidth?self.imageWidth.doubleValue:20].active = YES;
        [NSLayoutConstraint constraintWithItem:self.image attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeLeft multiplier:1 constant:0].active = YES;
        [NSLayoutConstraint constraintWithItem:self.image attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeRight multiplier:1 constant:0].active = YES;
    } else if (self.text && self.image) {
        //图文
        [self addSubview:self.imageView];
        [self addSubview:self.label];
